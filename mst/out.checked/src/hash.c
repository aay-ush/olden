...
static char *localmalloc(int size) : itype(_Array_ptr<char>) 
{
  char *blah;
  
  if (size>remaining) 
    {
      temp = (char *) malloc<char>(32768);
...
Hash MakeHash(int size, _Ptr<int (unsigned int)> map) // _ptr
{
  Hash retval = ((void *)0);
  int i;

  retval = (_Array_ptr<struct hash>) localmalloc(sizeof(*retval));
