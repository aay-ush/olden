...
{
  Vertex tmp = ((void *)0);   // Ptr
  Vertex helper _Checked[MAXPROC] = {((void *)0)};   // array
  int i;
  ...
  for (tmp = retval->vlist[j]; tmp; tmp=tmp->next) 
    {
      for (i=0; i<numproc*perproc; i++) 
        {
          int pn,offset,dist;
          Vertex dest = ((void *)0);  // Ptr
          Hash hash = ((void *)0);    // Ptr
...
Graph  MakeGraph(int numvert, int numproc) 
{
  ...
  int i,j;
  int count1;
  Vertex v : count(1) = ((void *)0);     // Ptr
  Vertex tmp : count(1) = ((void *)0);   // Ptr
  Vertex block : count(1) = ((void *)0); // ArrayPtr
  Graph retval = ((void *)0); // Ptr
  retval = (_Ptr<struct graph_st>)malloc<struct graph_st>(sizeof(*retval));
  for (i=0; i<MAXPROC; i++) 
    {
      retval->vlist[i]=NULL;
    }
  ...
  for (j=numproc-1; j>=0; j--) 
    {
      block = (_Array_ptr<struct vert_st>) malloc<struct vert_st>(perproc*(sizeof(*tmp)));
      v = NULL;
      for (i=0; i<perproc; i++) 
        {
